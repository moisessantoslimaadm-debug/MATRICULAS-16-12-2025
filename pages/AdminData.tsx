...
import { 
  ...
  Filter, LayoutGrid, CheckCircle2, ChevronDown
} from 'lucide-react';
            ...
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                <div className="bg-emerald-50 px-8 py-6 border-b border-emerald-100 flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-slate-900 tracking-tight">Editar Prontuário Escolar</h3>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Turma</label>
                            <input 
                                type="text" 
...
export const AdminData: React.FC = () => {
  ...
  const handleUpdateStudent = async (updated: RegistryStudent) => {
    ...
    try {
        ...
        addToast('Prontuário do aluno atualizado com sucesso.', 'success');
    } catch (e) {
        addToast('Falha ao sincronizar dados com o servidor.', 'error');
    }
  };
  ...
  const paginatedData = useMemo(() => {
    const data = activeTab === 'students' ? sortedStudents : schools.filter(s => s.name.toLowerCase().includes(searchTerm.toLowerCase()));
    const start = (currentPage - 1) * itemsPerPage;
    return data.slice(start, start + itemsPerPage);
  }, [activeTab, sortedStudents, schools, searchTerm, currentPage, itemsPerPage]);

  const totalPages = Math.ceil((activeTab === 'students' ? sortedStudents.length : schools.length) / itemsPerPage);
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-12">
            <div className="animate-in fade-in slide-in-from-left-4 duration-700">
                <h1 className="text-5xl font-black text-slate-900 tracking-tighter leading-none mb-3">Gestão de <span className="text-emerald-600">Rede Educacional</span></h1>
                <p className="text-slate-500 font-medium text-lg">Administração centralizada e controle nominal da matrícula digital.</p>
            </div>
            <div className="flex flex-wrap gap-4 animate-in fade-in slide-in-from-right-4 duration-700">
                 <button className="flex items-center gap-3 px-6 py-3.5 bg-emerald-50 text-emerald-700 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-emerald-100 hover:bg-emerald-100 transition shadow-sm">
                    <FileSpreadsheet className="h-4 w-4" /> Exportar Censo
                 </button>
                 <button onClick={() => setIsViewerOpen(true)} className="flex items-center gap-3 px-6 py-3.5 bg-white border border-slate-200 text-slate-400 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:text-emerald-600 transition shadow-sm">
                    <FileTerminal className="h-4 w-4" /> Logs SME
                 </button>
            </div>
        </header>

        {/* Abas e Filtro Principal */}
        <div className="bg-white p-6 rounded-[3rem] shadow-xl shadow-slate-100 border border-slate-100 mb-8 animate-in slide-in-from-bottom-4 duration-1000">
            <div className="flex flex-col lg:flex-row gap-6 justify-between items-center">
                ...
                <div className="relative w-full lg:w-[500px] group">
                    ...
                    <input 
                        type="text" 
                        placeholder={activeTab === 'students' ? "Buscar por Nome, CPF ou Protocolo..." : "Buscar por Nome da Escola..."}
                        value={searchTerm}
                        ...
                        <tr>
                            {activeTab === 'students' ? (
                                <>
                                    ...
                                    <SortHeader label="Nome do Estudante" field="name" />
                                    <SortHeader label="Unidade Alocada" field="school" />
                                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Atendimento</th>
                                ...
                                <>
                                    ...
                                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Capacidade</th>
                                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Controle Nomimal</th>
                                </>
                            )}
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                        {paginatedData.map((item: any) => (
                            <React.Fragment key={item.id}>
                                    ...
                                    {activeTab === 'students' ? (
                                        <>
                                            ...
                                            <td className="px-8 py-6">
                                                ...
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">CPF: {item.cpf || 'PENDENTE'}</p>
                                            </td>
                                            <td className="px-8 py-6">
                                                <div className="flex items-center gap-2">
                                            ...
                                            <td className="px-8 py-6">
                                                <div className="flex gap-2">
                                                    {item.specialNeeds && <div className="p-2 bg-pink-50 text-pink-600 rounded-xl border border-pink-100 animate-pulse" title="AEE Requerido"><HeartPulse className="h-4 w-4" /></div>}
                                                    {item.transportRequest && <div className="p-2 bg-amber-50 text-amber-600 rounded-xl border border-amber-100" title="Transporte Rural"><Bus className="h-4 w-4" /></div>}
                                                </div>
                                            </td>
                                            <td className="px-8 py-6">
                                                <span className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border shadow-sm ${item.status === 'Matriculado' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-blue-50 text-blue-700 border-blue-100'}`}>{item.status}</span>
                                            </td>
                                        ...
                                        <>
                                            ...
                                            <td className="px-8 py-8 text-center">
                                                <span className="font-black text-slate-900 text-xl">{item.availableSlots}</span>
                                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Vagas</p>
                                            </td>
                                            <td className="px-8 py-8 text-right">
                                                ...
                                                >
                                                    ...
                                                    {expandedSchoolId === item.id ? 'Ocultar Alunos' : 'Listar Alunos'}
                                                </button>
                                            </td>
                                        </>
                                    )}
                                ...
                                {/* Sub-aba: Quadro Nominal por Escola */}
                                                ...
                                                <div className="flex justify-between items-end mb-10">
                                                    ...
                                                    <div className="bg-white px-6 py-3 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
                                                        ...
                                                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                                            Matriculados: {students.filter(s => s.school === item.name).length}
                                                        </span>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                                        ...
                                                        <div key={studentInSchool.id} className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex justify-between items-center group/card hover:border-emerald-300 hover:shadow-xl transition-all duration-300">
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{studentInSchool.className || 'SEM TURMA ALOCADA'}</p>
                                                                </div>
                                                            </div>
                                                            {canEdit && (
                                                                <button 
                                                    ...
                                                    {students.filter(s => s.school === item.name).length === 0 && (
                                                        <div className="col-span-full py-16 text-center border-4 border-dashed border-slate-200 rounded-[3rem] text-slate-300 font-black uppercase tracking-[0.2em]">
                                                            Nenhum registro acadêmico vinculado a esta unidade
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
            ...
            <div className="bg-slate-50/50 px-12 py-8 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-8">
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        Página {currentPage} de {totalPages || 1} • {activeTab === 'students' ? sortedStudents.length : filteredStudents.length} Registros Totais
                    </span>
                </div>
        ...
        {/* Modal de Edição (Renderizado apenas se autorizado) */}
